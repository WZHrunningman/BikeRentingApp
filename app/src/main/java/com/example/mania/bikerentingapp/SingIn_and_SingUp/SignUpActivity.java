        ...
        if (v.getId() == R.id.btn_singup){
            if (Utils.isFastClick()){
                ...
                if (isUserName(userName) && isPassword(userName) && isMobile(number) && isEmail(email)){
                    ...
                    user.setEmail(email);
                    user.signUp(new SaveListener<Table_UserMassage>() {
                        @Override
                        public void done(Table_UserMassage table_userMassage, BmobException e) {
                            if (e == null){
                    ...
                    if (!isUserName(userName)){
                        ...
                        ed_number.setError("请输入正确的手机号码！");
                    }
                    else if (!isEmail(email)){
                        ed_email.setError("请输入正确邮箱！");
                    }
                }
            }
